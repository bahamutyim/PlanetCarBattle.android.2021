...
namespace emotitron.Network.Compression
{
	...
	public class AxisRange
	{
		...
		public void CalculateEncoder()
		{
            ...
            //Debug.Log(string.Format("axis={0},min={1},max={2},precision={3},bits={4},lowerBits={5},lowerTwoThirdsBits={6},lowerHalfBits{7},encoder{8},decoder{9},maxvalue{10} ",
            //    axis, min, max, precision, bits,
            //    lowerBits, lowerTwoThirdsBits, lowerHalfBits, encoder, decoder,
            //    maxvalue)
            //    );
		...
		public uint Encode(float val)
		{
            ...
            //uint tempReturn = (uint)Mathf.Clamp(((val - min) * encoder), 0, maxvalue);
            ////Debug.Log(string.Format("Encode {0}, before {1}, after {2}, maxvalue {3}, min {4}", encoder, val, tempReturn, maxvalue, min));
            //return tempReturn;
        }
		...
		public float Decode(uint val)
		{
           ...
           //float tempReturn = val * decoder + min;
           // //Debug.Log(string.Format("Decode {0}, before {1}, after {2}, min {3}", decoder, val, tempReturn, min));
           // return tempReturn;
        }
